...
import org.nigajuan.dynatree.model.Function;
...
public class DTreeRenderer extends Renderer {
    ...
    public static final String RENDER_TYPE = "org.nigajuan.dynatree.component.DTreeRenderer";
    ...
    public void encodeEnd(FacesContext context, UIComponent component) throws IOException {
        ...
        if (nodes != null) {
            children = DTUtils.getObjectWriter().writeValueAsString(nodes);
        } else {
            children = "null";
        }
    ...
    private static String buildScriptFromAjaxBehavior(ClientBehaviorContext behaviorContext, AjaxBehavior ajaxBehavior) {
        ...
        //execute ids
        scriptBuilder.append("'").append(resolveAndJoinIds(component, ajaxBehavior.getExecute())).append("',");
        //render ids
        scriptBuilder.append("'").append(resolveAndJoinIds(component, ajaxBehavior.getRender())).append("',");

        Map<String,Object> options = new HashMap<String, Object>();

        String onevent = ajaxBehavior.getOnevent();
        if (onevent != null){
            options.put("onevent" , new Function(onevent));
        }

        String onerror = ajaxBehavior.getOnerror();
        if (onerror != null){
            options.put("onerror" , new Function(onerror));
        }
        ...
        try {
            scriptBuilder.append(DTUtils.getObjectWriter().writeValueAsString(options));
        } catch (IOException e) {
